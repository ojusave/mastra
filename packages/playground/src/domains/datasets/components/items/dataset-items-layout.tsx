...
export function DatasetItemsLayout({ listSlot, detailPanelSlot, versionsPanelSlot }: DatasetItemsLayoutProps) {
  ...
  return (
    ...
    >
      <div className={cn('flex max-w-full flex-col gap-4 overflow-y-auto pt-4 pl-6', !showDetail && 'pr-6')}>
        {listSlot}
      </div>
      {showDetail && <div className="h-full min-h-0 pt-4 pr-6 pb-3">{detailPanelSlot}</div>}
