...
Calling `ctx.run()` from a Render task creates a chained task run. Each chained run executes in its own instance. You can set compute, timeout, and retries on that task alone.
...
:::note
Use Render CLI `v2.28.0` or later for the commands below. For other installation methods, see the [Render CLI docs](https://render.com/docs/cli?utm_source=partner\&utm_medium=partnerships\&utm_campaign=2026_partnership_mastra).
:::
...
Install Render SDK 1.x and the TypeScript runner:

```bash npm2yarn
npm install @renderinc/sdk@1 tsx
```
...
export const reviewDraft = task(
  ...
  async function reviewDraft(_ctx: TaskContext, draft: string, focus: string): Promise<Review> {
...
Render supplies `TaskContext` as the first argument to every task function, even when the task doesn't start child runs.

This guide uses the `flex` compute plan for every task. Choose another [compute plan](https://render.com/docs/workflows-defining?utm_source=partner\&utm_medium=partnerships\&utm_campaign=2026_partnership_mastra#compute-plan) if an operation needs more CPU or memory.

### Create the revision task

This task combines the feedback and produces a revised draft. It uses the same compute plan as each reviewer, with a longer timeout.
...
export const editorialPipeline = task(
  ...
  async function editorialPipeline(ctx: TaskContext, draft: string): Promise<{ draft: string }> {
    const focuses = ['technical clarity', 'structure and flow', 'reader usefulness']
    const reviews = await Promise.all(focuses.map(focus => ctx.run(reviewDraft, draft, focus)))
    return ctx.run(reviseDraft, draft, reviews)
  },
)
```
...
Those files are the complete pipeline. When you start the local development server, the Render CLI loads your model provider key from `.env`.

For an example with a web UI, see [render-examples/render-workflows-mastra](https://github.com/render-examples/render-workflows-mastra). That repository uses Render SDK 0.5.x. Follow this guide's `TaskContext` and `ctx.run()` syntax when using SDK 1.x.
...
The local server keeps runs and their logs in memory. Inspect the parent runs with:

```bash
render workflows tasks runs list editorial_pipeline --local
```
...
<Steps>
  ...
  <StepItem>
    ...
    In the [Render Dashboard](https://dashboard.render.com?utm_source=partner\&utm_medium=partnerships\&utm_campaign=2026_partnership_mastra), select **New > Workflow** and link the repository from the previous step. Then complete the creation form:
    ...
    Select **Deploy Workflow**. Render builds the project and registers `review_draft`, `revise_draft`, and `editorial_pipeline`, which then appear on the workflow's **Tasks** page.
  ...
  <StepItem>
    ...
    ```bash
    render workflows tasks runs start mastra-workflows/editorial_pipeline \
      --input='["Render Workflows runs long-running tasks outside the request lifecycle."]'
    ```
